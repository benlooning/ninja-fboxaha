...
Fbox.prototype.getAHADeviceName = function(deviceID, cb) {
  var self = this;
  ...
  requestValueAction (this._box.host, '/net/home_auto_overview.lua?sid=' + this.SID, function (error, resp) {
    ...
    if (resp == "inval" ){
	  ...
	  $ = cheerio.load(resp);
	  $('#tHAdevices .c1').each(function(index) { // find ID
	    if(index >= 1) { //skipp th (headline)
		  var strObj = parseInt($(this).html().split('id="')[1].split('"')[0].split('_')[1]);
		  if (strObj == deviceID)
		    indexFound = index;
		}
	  });      
	  if (indexFound != null) // get name
	    deviceName = $('#tHAdevices .c2').eq(indexFound).text();
	  cb(null, deviceName);
	}	
  });
};
