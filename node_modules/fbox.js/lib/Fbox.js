...
function Fbox(box) {
  ...
  this._box = box;
  this.boxInfo = { fritz: null, type: null, version: null, serial: null };
  this.SID = null;
  this.ahaDevices  = [];
  this.ahaDeviceIDs  = [];
  this.blockSID = false;
...
Fbox.prototype.validateBox = function(cb) {
  var self = this;
  getDataFromPath (this._box.host, '/jason_boxinfo.xml', function (error, info) {
...
Fbox.prototype.closeSession = function(SID, cb) {
  var self = this;
  getDataFromPath (this._box.host, '/login_sid.lua?logout=logout&sid=' + SID, function (error, info) {
...
Fbox.prototype.createSession = function(resp, cb) {
  ...
  getDataFromPath (this._box.host, path, function (error, info) {
...
Fbox.prototype.getAHADevices = function(cb) {
  var self = this;
  getDataFromPath (this._box.host, '/net/home_auto_query.lua?sid=' + this.SID + '&command=AllOutletStates&xhr=1', function (error, info) {
    ...
    if (info.Outlet_count){
	  for (var count = 0; count < parseInt(info.Outlet_count); count++) {
		if (self.ahaDeviceIDs.indexOf(info['DeviceID_' + parseInt(count + 1)].toString()) == -1) { // do not add if deviceID already exists
	      self.ahaDevices.push({ ID: info['DeviceID_' + parseInt(count + 1)], multiMeterTimer: null, outletTimer: null });
...
Fbox.prototype.getAHAMultiMeterState = function(MultiMeterID, cb) {
  var self = this;
  getDataFromPath (this._box.host, '/net/home_auto_query.lua?sid=' + this.SID + '&command=MultiMeterState&id=' + MultiMeterID + '&xhr=1', function (error, info) {
...
Fbox.prototype.switchOnOff = function(MultiMeterID, ValueToSet, cb) {
  var self = this;
  putDataToPath (this._box.host, '/net/home_auto_query.lua', 'sid=' + this.SID + '&command=SwitchOnOff&id=' + MultiMeterID + '&value_to_set=' + ValueToSet + '&xhr=1', function (error, info) {
...
Fbox.prototype.getSwitchState = function(MultiMeterID, ValueToSet, cb) {
  var self = this;
  getDataFromPath (this._box.host, '/net/home_auto_query.lua?sid=' + this.SID + '&command=SwitchOnOff&id=' + MultiMeterID + '&value_to_set=' + ValueToSet + '&xhr=1', function (error, info) {
...
function getDataFromPath(host, path, cb) {
  var url = {
	host: host,
	path: path,
	method: 'GET' //default
  ...
  var req = http.request(url, function(response) {
	...
	if (data.length <= 1 || data.indexOf('login.lua') > -1) { // SID is not valid
		cb('invalid SID', null);
	} else {
      try {
	    ...
	    } else if (response.headers['content-type'].indexOf('json') > -1) {
  ...
  req.on('error', function(error) { // connection error -> host unreachable
	//console.log('ERROR:	getDataFromPath	connection to host ' + url.host + ' failed: ' + error.message);
...
function putDataToPath(host, path, postData, cb) {
  var url = {
	host: host,
	path: path,
	method: 'POST', //default
  ...
  var req = http.request(url, function(response) {
	...
	response.on('end', function () {
	if (data.length <= 1) { // SID is not valid
		cb('invalid SID', null);
	} else {
      try {
	    //console.log (response.headers['content-type']);
...
function calculateResponse(Challenge, Password) {
  ...
  var ic8859 = new Iconv('utf-8', 'utf-16le');
  Response = (ic8859.convert(new Buffer(Response),'utf-8'));
  var resp = new Buffer(Response, 'utf-8');
